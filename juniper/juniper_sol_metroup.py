...
if __name__ == '__main__':
    ...
    # Now let's try to send the router a command
    remote_conn.send("\n")
    remote_conn.send("set route 0.0.0.0/0 interface ethernet0/7 gateway 213.128.91.73 preference 20 metric 12\n")
    ...
    # Now let's try to send the router a command
    remote_conn.send("\n")
    remote_conn.send('set vpn "SOL_VPN_S2S_IKE_SOLMETRO" bind interface tunnel.8\n')
